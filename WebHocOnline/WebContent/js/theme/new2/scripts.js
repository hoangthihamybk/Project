$(document).ready(function(){
    ...
    $('.view-more-addon').click(function () {
        ...
        if (height == '300px') {
            $('.view-more-addon').html('Thu gá»n<i class="fa fa-angle-up"></i>');
            $('.is-addon-ul').css('height', 'auto');
        } else {
            $('.view-more-addon').html('Xem thĂªm<i class="fa fa-angle-down"></i>');
...
function show_message($title, $message, $footer){
    ...
    if(typeof $footer == 'undefined'){
        $mModal.find('.modal-footer').html('<button type="button" class="btn btn-cancel" data-dismiss="modal">ÄĂ³ng</button>');
    }else{
...
function hideLearnPanel(){
    ...
    if (typeof player != 'undefined') {
        if (typeof is_html5_player != 'undefined' && is_html5_player) {
            player.pause();
        } else {
            player.stop();
        }
...
$(document).ready(function(){
    ...
    if ($('.course-detail-description').height() > 130) {
        var $moreDetail = $('<a href="#" class="cdd-more-bt">ThĂ´ng tin chi tiáº¿t &raquo;</a>');
    ...
    if($relateCourses.length > 5){
        ...
        var $rlMore = $('<div class="relate-course-more">Xem thĂªm</div>');
...
//        }
    ...
    issueForm.validate({
        ...
        messages:{
            link: {required: 'Link khĂ´ng há»£p lá»‡'},
            relate: {required: 'Báº¡n chÆ°a chon váº¥n Ä‘á»'},
            issue: {required: 'Báº¡n chÆ°a chá»n lá»—i'},
            email: {required: 'Mail khĂ´ng há»£p lá»‡', email: 'Mail khĂ´ng há»£p lá»‡'},
            phone: {required: 'Báº¡n chÆ°a nháº­p sá»‘ Ä‘iá»‡n thoáº¡i', phone: 'Sá»‘ Ä‘iá»‡n thoáº¡i khĂ´ng há»£p lá»‡'},
            title: {required: 'Báº¡n chÆ°a chá»n tiĂªu Ä‘á»'},
            time: {required: 'Thá»i gian khĂ´ng há»£p lá»‡'},
            content: {required: 'Báº¡n chÆ°a nháº­p ná»™i dung'}
        },
        errorElement: 'label',
        ...
        if($this.hasClass('with-play-scorm')){
            if(typeof $loadvideo != 'undefined'){
                if($this.hasClass('confirm')){
                    //hiá»ƒm thá»‹ thĂ´ng bĂ¡o trá»« sá»‘ láº§n xem
                            ...
                            success: function($response) {
                                ...
                                if($jdata){
                                    ...
                                    if($jdata.status == 'success'){
                                        ...
                                        show_message('NĂ¢ng cáº¥p há»‡ thá»‘ng', $jdata.message);
                                    }
                                }else{
                                    alert('Vui lĂ²ng liĂªn há»‡ Hotline: 1900 6933');
                                }
                        ...
                        success: function($res){
                            if($res.login == 'true'){
                                ...
                                if ($res.exeaction == 'doTest') {
                                    ...
                                    //Náº¿u lĂ  hiá»‡n thĂ´ng bĂ¡o
                                    if($res.showmess){
                                        $mModal.find('.warning-des').html($res.message);
                                        if($res.messaget){
                                            $mModal.find('.modal-title').html($res.messaget);
                                        }else{
                                            $mModal.find('.modal-title').html('ThĂ´ng bĂ¡o');
                                        }
                                        if($res.messagef){
                                            $mModal.find('.modal-footer').html($res.messagef);
                                        }
                                        ...
                                        if($res.exeaction){
                                            switch($res.exeaction){
                                                case 'freeLearnBooking':
                                                    $modalIC.data('title', 'Cáº­p nháº­t thĂ´ng tin Ä‘Äƒng kĂ½ há»c miá»…n phĂ­');
                                                ...
                                                default:
                                                    if(typeof $freescorm != 'undefined' && $freescorm){
                                                        $modalIC.data('title', 'Cáº­p nháº­t thĂ´ng tin Ä‘á»ƒ há»c miá»…n phĂ­');
                                                    }else{
                                                        $modalIC.data('title', 'Cáº­p nháº­t thĂ´ng tin Ä‘á»ƒ vĂ o há»c');
                                                    }
    ...
    $('#playscromvideo').click(function(){
        $('.scorm-header.with-play-scorm').removeClass('confirm').trigger('click');
        //Náº¿u chá»n khĂ´ng hiá»ƒn thá»‹ thĂ´ng bĂ¡o ná»¯a
    ...
    $('body').on('click', '.extent-trial2', function(){
        ...
        $.post('/su-kien/khoa-co-ban-10/extent-trial.php', {freeLearnId: freeLearnId}, function (response) {
            ...
            if (response.status == 'success'){
                alert.find('.modal-title').text('Gia háº¡n thĂ nh cĂ´ng');
                alert.find('.warning-des').html(response.message);
                alert.find('.modal-footer').html('');
                alert.modal('show');
            } else {
                alert.find('.modal-title').text('Gia háº¡n chÆ°a thĂ nh cĂ´ng');
                alert.find('.warning-des').html(response.message);
                alert.find('.modal-footer').html('');
                alert.modal('show');
            }
