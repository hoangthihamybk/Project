...
$(document).ready(function () {
    ...
    modalList.on('click', '.extent-trial', function (e) {
        ...
        $.post('/su-kien/khoa-co-ban-10/extent-trial.php', {freeLearnId: freeLearnId}, function (response) {
            ...
            if (response.status == 'success'){
                alert.find('.modal-title').text('ThĂ nh cĂ´ng');
                alert.find('.warning-des').html(response.message);
                alert.modal('show');
                expiredButton.removeClass('extent-trial').attr('href', response.url).attr('target', '_blank').unbind('click').text('Há»c ngay');
                expiredButton.parents('tr').find('.expired').html(response.time);
            } else {
                alert.find('.modal-title').text('Lá»—i gia háº¡n');
                alert.find('.warning-des').html(response.message);
                alert.modal('show');
            }
        }, 'JSON');
                ...
                if(response.count) {
                    ...
                    for (var i in response.registedCourses) {
                        ...
                        if (response.registedCourses[i].isExpired) {
                            button += '<a class="extent-trial btn btn-fl" href="#" data-id="'+ i +'">Gia háº¡n</a>';
                        } else {
                            button += '<a class="btn btn-fl" href="'+ response.registedCourses[i].url +'" data-id="'+ i +'">Há»c ngay</a>';
                        }
                    ...
                    tableBody.append('<tr><td colspan="4"><div class="info">Báº¡n chÆ°a Ä‘Äƒng kĂ½ khĂ³a há»c miá»…n phĂ­ nĂ o<br/>Vui lĂ²ng chá»n khĂ³a há»c bĂªn dÆ°á»›i!</div></td></tr>');
                }
                        ...
                        if (typeof response.unregistedCourses[i] !== 'string') {
                            ...
                            for (var j in response.unregistedCourses[i]) {
                                ...
                                var package = '<tr>' +
                                    ...
                                    '<td style="text-align: center"><a target="_blank" href="'+ response.unregistedCourses[i][j].url +'" data-package="'+ j +'" class="btn btn-primary regist-course">Há»c miá»…n phĂ­ khĂ³a nĂ y</a>';

                                    if (response.unregistedCourses[i][j].upgradeUrl) {
                                        package += '<a style="margin-left: 5px" target="_blank" href="'+ response.unregistedCourses[i][j].upgradeUrl +'" data-package="'+ j +'" class="btn btn-success regist-course">NĂ¢ng cáº¥p</a>';
                                    }
    ...
    $('.learntrial-form').validate({
        ...
        messages: {
            code: {
                required: 'Báº¡n chÆ°a nháº­p mĂ£ code'
            },
        },
        errorElement: 'div',
                    ...
                    success: function ($response) {
                        switch($response.status){
                            case 'success':
                                ...
                                $mModal.find('.modal-title').html('XĂ¡c nháº­n thĂ nh cĂ´ng');
                            ...
                            case 'fail':
                                var $mModal = $('#messageModal');
                                $mModal.find('.warning-des').html($response.message);
                                $mModal.find('.modal-title').html('Lá»—i');
                                $mModal.find('.modal-footer').html('<button type="button" class="btn btn-cancel" data-dismiss="modal">ÄĂ³ng</button>');
                                $mModal.modal('show');
                            break;
